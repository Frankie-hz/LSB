...
: m_PRequester(PRequester)
{
    ...
    auto*      PZoneInstance = dynamic_cast<CZoneInstance*>(zoneutils::GetZone(instanceData.instance_zone));
    if (!PZoneInstance)
    {
    ...
    m_PZone     = PZoneInstance;
    m_PInstance = PZoneInstance->CreateInstance(instanceId);
}
