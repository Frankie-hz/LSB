describe('Mob self buffs', function()
    ...
    local casts
    local originalUseEnhancingSpell
    local originalUseEnhancingSong

    local function recordCasts()
        casts = {}
        originalUseEnhancingSpell = xi.spells.enhancing.useEnhancingSpell
        originalUseEnhancingSong  = xi.spells.enhancing.useEnhancingSong
        xi.spells.enhancing.useEnhancingSpell = function(caster, target, spell)
            ...
            return originalUseEnhancingSpell(caster, target, spell)
        end
        xi.spells.enhancing.useEnhancingSong = function(caster, target, spell)
            table.insert(casts, { caster = caster:getID(), target = target:getID(), spell = spell:getID() })
            return originalUseEnhancingSong(caster, target, spell)
        end
    end

    local function stopRecording()
        if originalUseEnhancingSpell then
            xi.spells.enhancing.useEnhancingSpell = originalUseEnhancingSpell
            originalUseEnhancingSpell = nil
        end
        if originalUseEnhancingSong then
            xi.spells.enhancing.useEnhancingSong = originalUseEnhancingSong
            originalUseEnhancingSong = nil
        end
    end
