...
describe('Battlefield entry', function()
    ...
    it('does not admit a player into the battlefield of a party they left', function()
        ...
        -- Zoning drops the clearance, then the drifter moves to a new party
        drifter:gotoZone(xi.zone.RIVERNE_SITE_A01)
        drifter:gotoZone(xi.zone.MONARCH_LINN)
        assert(not drifter:hasStatusEffect(xi.effect.BATTLEFIELD), 'zoning should drop clearance')
