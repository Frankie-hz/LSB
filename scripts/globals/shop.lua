...
---@return integer
xi.shop.getPriceRank = function(player, fameArea)
...
---Price the vendor charges the player
---@param player CBaseEntity
---@param standardPrice integer
---@param fameArea xi.fameArea
---@return integer
xi.shop.getBuyPrice = function(player, standardPrice, fameArea)
    local priceRank = xi.shop.getPriceRank(player, fameArea)
    return math.max(1, math.floor(standardPrice * (111 - priceRank) / 100))
end
...
xi.shop.general = function(player, stock, log)
    ...
    for _, stockItem in ipairs(stock) do
        ...
        -- Nation fame price rank adjustment, x0.90 (max fame) to x1.10 (min fame)
        if log ~= -1 then
            price = xi.shop.getBuyPrice(player, price, log)
        end
