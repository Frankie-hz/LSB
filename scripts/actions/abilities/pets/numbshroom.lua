...
-- Note: Values copied 1:1 from the mob version (scripts/actions/mobskills/numbshroom.lua)
...
abilityObject.onAbilityCheck = function(player, target, ability)
    ...
    if pet == nil or pet:getAnimationSub() ~= 1 then
...
abilityObject.onPetAbility = function(target, pet, petskill, owner, action)
    ...
    if xi.mobskills.processDamage(pet, target, petskill, action, info) then
        ...
        xi.mobskills.mobStatusEffectMove(pet, target, xi.effect.PARALYSIS, 25, 0, 180)
    end

    petskill:setFinalAnimationSub(2)
